...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    auto& logo = fastbootd_logo_enabled_ ? fastbootd_logo_ : candy_logo_;
...
int ScreenRecoveryUI::SelectMenu(const Point& point) {
  ...
  if (menu_) {
    if (!menu_->IsMain()) {
      // Back arrow hitbox
      const static int logo_width = gr_get_width(candy_logo_.get());
      const static int logo_height = gr_get_height(candy_logo_.get());
